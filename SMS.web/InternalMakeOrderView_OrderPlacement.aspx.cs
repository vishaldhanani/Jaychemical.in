...
public partial class InternalMakeOrderView_OrderPlacement : System.Web.UI.Page
{
    #region Variable
    ...
    protected void lb_Satistic_Click(object sender, EventArgs e)
    {
        try
        {
            //passed Sales Order No to JavaScript to popup statistic page.
            lbl_home_logo.Visible = false;
    ...
    protected void lb_Preview_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //Passed Sales Order no to JavaScript function for popup Preview for SalesOrder Report.
            string SalesOrder = Convert.ToString(ViewState["SalesOrder"]);
    ...
    private void BindNoSeriesCodeLocationWise()
    {
        try
        {
            List<ProductCat> list = ProductCat.ListNoseriesCodeLocationWiseData(drplocationcode.SelectedValue);
            if (list != null && list.Count > 0)
            {
                drpNoseries.SelectedValue = list[0].NoSeries;                
            }
