...
public partial class ActCustomerDueInNext7Days : System.Web.UI.Page
{
    #region PageEvents
    ...
    private void BindCustomerWiseBalance()
    {
        try
        {
            List<CustomerwiseBalance> list = CustomerwiseBalance.ListDueInNext7Days(SessionManager.GetAgentCode(HttpContext.Current));
    ...
    #region Events
    protected void rpt_Customer_DueInNext7Days_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (a_link != null)
                {
                    a_link.HRef = "ActDueInNext7DaySummaryNew.aspx?Customer=" + Convert.ToString(DataBinder.Eval(e.Item.DataItem, "CustomerNo")) + "&Name=" + Convert.ToString(DataBinder.Eval(e.Item.DataItem, "Name")) + "";
                }
            }
        }
        catch (Exception ex)
