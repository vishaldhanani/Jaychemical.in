...
public partial class OrderModificationView : System.Web.UI.Page
{

    #region Variable
    BlanketOrders_Service objbo = new BlanketOrders_Service();
    string inoviceno;
    decimal TotalQty = 0, TotalPrice = 0, TotalOSQTY = 0;
    DataTable dtCart;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Convert.ToString(Request.QueryString["BlanketOrderNo"]) != "" && Convert.ToString(Request.QueryString["BlanketOrderNo"]) != String.Empty)
                {
                    lbl_blanketOrderNo.Text = Convert.ToString(Request.QueryString["BlanketOrderNo"]);
                }
    ...
    private void BindCustomerInfo()
    {
        try
        {
            Qtm.Lib.OrderModifiCustomerInfo obj = Qtm.Lib.OrderModifiCustomerInfo.Find(SessionManager.GetItemCategoryCode(HttpContext.Current), Request["OrderNo"].ToString());
            if (obj != null)
            {
            ...
            Qtm.Lib.OrderModifiCustomerInfo objc = Qtm.Lib.OrderModifiCustomerInfo.FindConsignee(SessionManager.GetItemCategoryCode(HttpContext.Current), Request["OrderNo"].ToString());
            if (objc != null)
            {
    ...
    protected void rpt_FinalCart_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                TotalOSQTY += Convert.ToInt32(DataBinder.Eval(e.Item.DataItem, "OutStandingQty"));
            }
            ...
            if (e.Item.ItemType == ListItemType.Footer)
            {
                ...
                Label l_TotalOSQTY = e.Item.FindControl("l_TotalOSQTY") as Label;
                ...
                if (l_TotalQty != null && l_TotalPrice != null)
                {
                    ...
                    l_TotalOSQTY.Text = TotalOSQTY.ToString("0");
                }
            }
        }
        catch (Exception ex)
    ...
    protected void btn_edit_Command(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            if (ItemNo != null)
            {
                ...
                {
                    Response.Redirect("OrderModificationItemsDyes.aspx?ProductCode=" + ItemNo.Value + "&OrderNo=" + Convert.ToString(Request["OrderNo"]) + "&LineNo=" + LineNo.Value + "&BlanketOrderNo=" + Convert.ToString(Request["BlanketOrderNo"]) + "&PostingDate=" + Convert.ToString(Request["PostingDate"]), false);

                }
                else
                {
                    Response.Redirect("OrderModificationItems.aspx?ProductCode=" + ItemNo.Value + "&OrderNo=" + Convert.ToString(Request["OrderNo"]) + "&LineNo=" + LineNo.Value + "&BlanketOrderNo=" + Convert.ToString(Request["BlanketOrderNo"]) + "&PostingDate=" + Convert.ToString(Request["PostingDate"]), false);
                }
            }
        }
        catch (Exception ex)
